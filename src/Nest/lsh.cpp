...
LshBuilder::LshBuilder(const char* conf_file)
{
	global_lsh_param = initLshParam(conf_file);
	if (NULL == global_lsh_param){
		fprintf(stderr, "init lsh parameters failed.\n");
		return;
	}
}

LshBuilder::~LshBuilder()
{
	freeLshParam(global_lsh_param);
}

LshParam* LshBuilder::get_global_lsh_param(void)
{
	return global_lsh_param;
}
 ...
 * 成功返回初始化的配置， 失败返回NULL
 */
LshParam* LshBuilder::initLshParam(const char *config_file)
{
...
void LshBuilder::printConfig(LshConfig *conf)
{
	char *lsh_type = NULL;
 ...
 * 失败返回-1, 成功返回0
 */
int LshBuilder::loadConfigFile(const char* filename, LshConfig *conf)
{
 ...
 * 成功返回正数，失败返回负数
 */
int LshBuilder::initHashFamily(LshConfig *conf, HashFamily *hash_family, UniformHashFunction *uhf, int num)
{
 ...
 * @range_end: 范围结束点
 */
float LshBuilder::getUniformRandom(float range_start, float range_end)
{
	float tmp;
	float ret;
 ...
 * 得到满足高斯分布的浮点数
 */
float LshBuilder::getGaussianRandom()
{
	float x1, x2;
	float z;
 ...
 * @range_end: 范围结束点
 */
unsigned int LshBuilder::getRandomUns32(unsigned int range_start, unsigned int range_end)
{
	unsigned int ret;
	unsigned int tmp;
	if(range_start > range_end)
 ...
 * 成功返回正数，失败返回负数
 */
int LshBuilder::initUniformHashFunction(UniformHashFunction *uhf, LshConfig *conf)
{
...
int LshBuilder::comp(const void *a, const void *b)
{
	double a1 = *(double*)a;
	double b1 = *(double*)b;
 ...
 */
unsigned long LshBuilder::computeLsh(void *data, void *other)
{
	...
	if(conf->lsh_type == E2LSH) {
		...
		qsort(tmp, k, sizeof(double), LshBuilder::comp);
 ...
 * 释放哈希函数族的内存空间
 */
void LshBuilder::freeHashFamily(HashFamily *hash_family, int num)
{
 ...
 * 释放LSH参数的内存空间
 */
void LshBuilder::freeLshParam(LshParam *lsh_param)
{
